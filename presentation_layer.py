...
def render_stock_block(stock, include_market: bool, market_block: str) -> str:
    ...
    lines = [
        ...
        f"60日高値：{fmt_price(stock.get('recent60_high'))}",
        f"60日安値：{fmt_price(stock.get('recent60_low'))}",
        f"現在レンジ：{fmt_pct_plain((stock.get('recent60_range_position') or 0) * 100) if stock.get('recent60_range_position') is not None else 'N/A'}（{stock.get('recent60_range_zone', 'N/A')}）",
        "",
        "■流れ",
