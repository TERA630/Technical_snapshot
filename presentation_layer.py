...
def render_stock_block(stock, include_market: bool, market_block: str) -> str:
    ...
    lines = [
        ...
        f"四半期データ判定 {stock.get('quarterly_data_status', 'N/A')}（取得期数 {stock.get('quarterly_periods_count', 'N/A')} / 最新期末 {stock.get('quarterly_latest_period', 'N/A')}）",
        f"直近四半期営業利益率 {fmt_pct_no_sign_jp(stock.get('op_margin_q_latest'))}",
        f"直近四半期営業成長率（前年比） {fmt_pct_no_sign_jp(stock.get('op_growth_q_yoy'))}",
